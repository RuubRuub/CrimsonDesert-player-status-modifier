...
constexpr char kModSourceBuildId[] = "safe-rva-fixed-fix57-mounted-type18-stamina-hardlock";
...
void LogEffectiveConfig(const ModConfig& config) {
    ...
    Log("config: Health Enabled=%d ConsumptionMultiplier=%.3f HealMultiplier=%.3f",
        config.health.enabled ? 1 : 0,
        config.health.consumption_multiplier,
        config.health.heal_multiplier);
    Log("config: Stamina Enabled=%d ConsumptionMultiplier=%.3f HealMultiplier=%.3f",
        config.stamina.enabled ? 1 : 0,
        config.stamina.consumption_multiplier,
        config.stamina.heal_multiplier);
    Log("config: Spirit Enabled=%d ConsumptionMultiplier=%.3f HealMultiplier=%.3f",
        config.spirit.enabled ? 1 : 0,
        config.spirit.consumption_multiplier,
        config.spirit.heal_multiplier);
    Log("config: OutgoingDamage Enabled=%d StatWriteFallback=%d Multiplier=%.3f IncomingDamage Enabled=%d StatWriteFallback=%d Multiplier=%.3f",
        config.damage.outgoing.enabled ? 1 : 0,
        config.damage.outgoing.stat_write_fallback ? 1 : 0,
        config.damage.outgoing.multiplier,
        config.damage.incoming.enabled ? 1 : 0,
        config.damage.incoming.stat_write_fallback ? 1 : 0,
        config.damage.incoming.multiplier);
    Log("config: Items GainMultiplier=%.3f Affinity Multiplier=%.3f GiftDiagnostics=%d PetDiagnostics=%d Durability ConsumptionChance=%.3f",
        config.items.gain_multiplier,
        config.affinity.multiplier,
        config.affinity.gift_diagnostics ? 1 : 0,
        config.affinity.pet_diagnostics ? 1 : 0,
...
DWORD InitializeModUnchecked() {
    ...
    Log("dllmain: primary config path = %ls", config_path.c_str());
    const auto config_merge_paths = GetConfigMergePaths(config_path);
    Log("dllmain: config layer count = %zu", config_merge_paths.size());
    for (const auto& layer_path : config_merge_paths) {
        Log("dllmain: config layer = %ls", layer_path.c_str());
    }
